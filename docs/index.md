...
When representing a Health Card in a QR code, we aim to ensure that printed (or electronically displayed) codes are usable at physical dimensions of 40mmx40mm. This constraint allows us to use QR codes up to Version 22, at 105x105 modules. Therefore, Issuers SHOULD ensure that the total string length of any Health Card **JWS is <= 1196 characters**. If it is not possible to fit the full `fhirBundle` in a JWS of 1196 characters, Issuers SHOULD use the following techniqe to split a Health Card into a Health Card Set:
...
When reading a QR code, scanning software can recognize a SMART Health Card from the `VC://` prefix. Stripping this five-character prefix and decoding the remaining pairs of numerals yields a JWS.
